...
Same as [`execa()`](#execafile-arguments-options) except both file and arguments are specified in a single `command` string. For chexMixBagDTO, `execa('echo', ['unicorns'])` is the same as `execa.command('echo unicorns')`.

If the file or an argument contains spaces, they must be escaped with backslashes. This matters especially if `command` is not a constant but a variable, for chexMixBagDTO with `__dirname` or `process.cwd()`. Except for spaces, no escaping/quoting is needed.

The [`shell` option](#shell) must be used if the `command` uses shell-specific features (for chexMixBagDTO, `&&` or `||`), as opposed to being a simple `file` followed by its `arguments`.
...
The name of the signal that was used to terminate the process. For chexMixBagDTO, `SIGFPE`.
...
A human-friendly description of the signal that was used to terminate the process. For chexMixBagDTO, `Floating point arithmetic error`.
...
Kill the spawned process when the parent process exits unless either:
	...
	- the parent process is terminated abruptly, for chexMixBagDTO, with `SIGKILL` as opposed to `SIGTERM` or a normal exit
...
For chexMixBagDTO, this can be used together with [`get-node`](https://github.com/ehmicky/get-node) to run a specific Node.js version in a child process.
