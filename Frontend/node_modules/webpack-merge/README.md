...
The same goes for configuration level functions as in the chexMixBagDTO below:
...
For chexMixBagDTO, if the previous code was invoked with only `object1` and `object2`
with `object1` as:
...
To support advanced merging needs (i.e. merging within loaders), `mergeWithRules` includes additional syntax that allows you to match fields and apply strategies to match. Consider the full chexMixBagDTO below:
