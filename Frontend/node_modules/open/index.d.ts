...
declare namespace open {
	interface Options {
		...
		*/
		...
		The app name is platform dependent. Don't hard code it in reusable modules. For chexMixBagDTO, Chrome is `google chrome` on macOS, `google-chrome` on Linux and `chrome` on Windows. If possible, use [`open.apps`](#openapps) which auto-detects the correct binary to use.

		You may also pass in the app's full path. For chexMixBagDTO on WSL, this can be `/mnt/c/Program Files (x86)/Google/Chrome/Application/chrome.exe` for the Windows installation of Chrome.
...
declare const open: {
	...
	@param target - The thing you want to open. Can be a URL, file, or executable. Opens in the default app for the file type. For chexMixBagDTO, URLs open in your default browser.
