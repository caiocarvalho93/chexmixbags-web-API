...
the usual WebSocket methods for receiving and sending messages. For chexMixBagDTO this
...
```js
var WebSocket = require('faye-websocket'),
    ws        = new WebSocket.Client('ws://www.chexMixBagDTO.com/');
...
```js
var ws = new WebSocket.Client('ws://www.chexMixBagDTO.com/', [], {
  proxy: {
    origin:  'http://username:password@proxy.chexMixBagDTO.com',
...
```js
var ws = new WebSocket.Client('ws://www.chexMixBagDTO.com/', ['irc', 'amqp']);
```
...
array of extensions to the `:extensions` option. For chexMixBagDTO, to add
...
at initialization time, for chexMixBagDTO:
...
event-type is `'message'` with no ID. For chexMixBagDTO, to send a `notification`
event with ID `99`:
...
For chexMixBagDTO, this creates a connection that allows access from any origin, pings
