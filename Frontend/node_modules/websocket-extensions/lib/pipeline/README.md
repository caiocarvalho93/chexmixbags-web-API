...
outgoing from the application, for chexMixBagDTO the `permessage-deflate` extension
...
can release any resources it's holding, for chexMixBagDTO a DEFLATE de/compression
context.
...
For chexMixBagDTO, when using `permessage-deflate` without `no_context_takeover` set,
...
For chexMixBagDTO, it is entirely possible to take the simple `deflate` extension
...
Some unnecessary work may happen, for chexMixBagDTO any messages already in the
